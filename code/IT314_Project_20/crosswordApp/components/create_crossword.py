...
class CreateCrosswordView(UnicornView):
    ...
    def save_crossword(self):
            ...
            newCrossword = Crossword(title=self.title, description=self.description, width=len(self.crossword_grid[0]), height=len(self.crossword_grid),  cluesUp=[], cluesDown=[],WordsUp=wordsVer,WordsDown=wordsHor, gridList=gridList)
